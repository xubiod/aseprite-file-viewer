...
func _process_custom_fx(char_fx: CharFXTransform) -> bool:
	...
	var progress0 : float = wrapf(((float(char_fx.relative_index) * stretch) / float(char_fx.glyph_count)) + (char_fx.elapsed_time / spd), 0, float(len(colour)))
	var progress1 : float = wrapf(((float(char_fx.relative_index + 1) * stretch) / float(char_fx.glyph_count)) + (char_fx.elapsed_time / spd), 0, float(len(colour)))
