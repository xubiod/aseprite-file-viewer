class_name FrameDisplayItem extends ColorRect
...
func _process(_delta: float) -> void:
	...
	color.a = lerpf(color.a, target_alpha, 0.3)
...
func _range_changed(from: int, to: int) -> void:
	color = Color(Color.WHITE, color.a) if frame_based >= from && frame_based <= to else Color(Color.DIM_GRAY, color.a)
